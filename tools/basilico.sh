...
function show_help {
  echo './tools/basilico.sh -i input.icb -d output/ [-o -w]'
  echo './tools/basilico.sh -h'
...
echo
icm ./scripts/basilico/settags.icm if=${IFILE} of=${TAGGED} || exit 1
[ -e ${TAGGED} ] || exit 1

echo
icm ./scripts/basilico/ligclustering.icm if=${TAGGED} of=${TLIG} ${OBJECTMODE} || exit 1
[ -e ${TLIG} ] || exit 1

echo
icm ./scripts/basilico/stre.icm if=${TAGGED} tlig=${TLIG} of=${STRE} ${OBJECTMODE} || exit 1
[ -e ${STRE} ] || exit 1
...
echo
icm ./scripts/basilico/mkweb.icm if=${STRE} osvg=${SVGROOT} ohtml=${HTML} otsv=${TSV} || exit 1
[ -e ${SVGROOT} ] || exit 1
